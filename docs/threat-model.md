...
- **File Permissions:** `.env` files are plain text. Protect them with `chmod 600`.
...
| FD leakage to child processes | `O_CLOEXEC` on pipes |
...
| Threat | Reason |
|--------|--------|
| **Secrets at rest** | `.env` files are plain text. Protect with file permissions. |
...
- The human operator, not the AI, should control `--env` and `--no-shell`

The redaction side-channel oracle cannot be closed by any flag. If this is a concern, do not allow the AI to construct commands that compare secrets to guesses.
